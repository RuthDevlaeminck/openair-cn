...
char                                   *
fdiam_experimental_retcode_2_string (
  uint32_t ret_code)
{
  switch (ret_code) {
    /*
...
int
fdiam_parse_experimental_result (
  struct avp *avp,
  fdiam_experimental_result_t * ptr)
{
  struct avp_hdr                         *hdr;
  ...
  while (child_avp) {
    ...
    case AVP_CODE_EXPERIMENTAL_RESULT_CODE:
      OAILOG_ERROR (LOG_FDIAM, "Got experimental error %u:%s\n", hdr->avp_value->u32, fdiam_experimental_retcode_2_string (hdr->avp_value->u32));

      if (ptr) {
        *ptr = (fdiam_experimental_result_t) hdr->avp_value->u32;
      }
